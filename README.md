...
Agent 以 **plan(只读)权限模式**运行,仅允许 `Read` / `Glob` / `Grep` / `Bash` 工具,
专注代码分析、引用具体文件路径与行号,**不修改任何文件**。
...
| 接口 | 方法 | 说明 |
| --- | --- | --- |
| `/ask` | POST | 同步问答,返回 `{answer, cost_usd, num_turns}` |
...
```jsonc
{ "question": "你的代码问题", "max_turns": 10 }   // max_turns 可选,默认 10
```
...
| --- | --- | --- |
| `token` | `{"text": "..."}` | 回答的文本增量 |
| `done`  | `{"answer","cost_usd","num_turns"}` | 成功结束,与 `/ask` 返回结构一致 |
| `error` | `{"error": "..."}` | 失败 |
...
```text
.
├── server.py         # FastAPI 服务:三个接口 + Agent 调用逻辑
